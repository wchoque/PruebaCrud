...
use App\Http\Controllers\Sunat\Zipeoxml;
	...
	class ServicioSunatDocumentosProduccion
	{
			...
			public function RespuestaSincronoConsultarConstanciaDeRecepcion($ticket)
			{
				$user='1111111111MODDATOS';
				$pass='MODDATOS';
				$service = 'https://www.sunat.gob.pe/ol-it-wsconscpegem/billConsultService?wsdl'; 
				$headers = new CustomHeaders($user, $pass); 
				$client = new \SoapClient($service, [ 'cache_wsdl' => WSDL_CACHE_NONE, 'trace' => TRUE , 'soap_version' => SOAP_1_1 ] ); 
				$client->__setSoapHeaders([$headers]); 
				$fcs = $client->__getFunctions();
				//$fileName='10292703151-03-B001-0004078.zip';
				//$contentfilezip=file_get_contents('XML/'.$fileName);
				$params = array( 'rucComprobante' => $ticket,'tipoComprobante' => $ticket,'serieComprobante' => $ticket,'numeroComprobante' => $ticket ); 
				//$resultado = $client->sendBill($params);
				
				$resultado = $client->getStatusCdr($params);
				//print_r($resultado);
				//echo $fileName;
